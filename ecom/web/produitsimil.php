...
$reponse=$bdd->query("SELECT `type` ,`prix` FROM produits  WHERE `id`=$idprd ");
...
if($data=$reponse->fetch())
{
	...
	$reponse=$bdd->query("SELECT id,nom,prix,description FROM produits WHERE NOT `id`=$idprd AND type='$type' AND prix>'$prixinf' AND prix<'$prixsup' LIMIT 3");
		...
		?>
              ...
              <div class="card col" style="border: 2px ; border-radius: 8px; margin-left: 5px; margin-right: 5px;">
                 ...
                 <div  id="cardinfo" class="card-body text-center pb-0" style="margin-bottom: 10px;">
                   ...
                   <p class="card-text" hidden><?php echo substr($data['description'],0,40)?>...</p>
